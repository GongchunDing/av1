...
//by dgc
int av1_openvino_restore_cnn_tflite(int qindex, const uint8_t *dgd, int width,
                                    int height,
                        int dgd_stride, uint8_t *rst, int rst_stride,
                        int num_threads, int is_intra_only, int is_luma,
                        const uint8_t *src, int src_stride, CRLCInfo *ci,
                        int frameType);
...
void av1_openvino(AV1_COMMON *cm, int num_threads,
                  YV12_BUFFER_CONFIG *source_frame, int plane_from,
                  int plane_to);
