...
extern "C" int av1_restore_cnn_img_tflite(int qindex, const uint8_t *dgd,
  ...
  for (int r = 0; r < height; ++r) {
    for (int c = 0; c < width; ++c) {
      ...
      //cout << +static_cast<uint8_t>(rst[r * rst_stride + c]) << "  ";
    }
  }
...
std::vector<Blob::Ptr> readInputBlobs(std::vector<UString> &data, size_t width,
                                      size_t height) {
  ...
  const InferenceEngine::TensorDesc uv_plane_desc(
      InferenceEngine::Precision::U8, { 1, 2, height / 2, width / 2 },
      InferenceEngine::Layout::NHWC);
...
// by dgc
extern "C" int av1_openvino_restore_cnn_tflite(
  ...
  string modelXML =
      "D:\\Program "
      "Files\\openvino\\openvino_2021.4.752\\deployment_tools\\model_"
      "optimizer\\new_zxecf_20211130_202.xml";
  string modelBin =
      "D:\\Program "
      "Files\\openvino\\openvino_2021.4.752\\deployment_tools\\model_"
      "optimizer\\new_zxecf_20211130_202.bin";

  InferenceEngine::CNNNetwork network;
  InferenceEngine::ExecutableNetwork executable_network;

  Core core;
  network = core.ReadNetwork(modelXML, modelBin);

  //cout << "==========ReadNetwork  success============" << endl;
  ...
  for (auto &item : input_info) {
    ...
    SizeVector new_input_shape;
    std::string input_name;
    std::tie(input_name, new_input_shape) =
        *inputShapes.begin();  // let's consider first input only
    new_input_shape[0] = 1;  // set batch size to the first input dimension
    new_input_shape[2] = height;  // changes input height to the image one
    new_input_shape[3] = width;   // changes input width to the image one
    inputShapes[input_name] = new_input_shape;
    network.reshape(inputShapes);
    
    // set color format
    /*input_data->getPreProcess().setColorFormat(
  ...
  for (auto &item : input_info) { 
    ...
    if (!minput) {
      cout << "We expect MemoryBlob from inferRequest_regular, but by fact we "
    ...
    auto data =
        minputHolder.as<PrecisionTrait<Precision::FP32>::value_type *>();

    size_t num_channels = input->getTensorDesc().getDims()[1];
    size_t h = input->getTensorDesc().getDims()[2];
    size_t w = input->getTensorDesc().getDims()[3];
    size_t image_size = h * w;
    //cout << "input channel  " << num_channels << " height " << h << "width "<<w << endl;
    // NCHW
    //cout << "dgd stride = " << dgd_stride << endl;
    
    for (size_t row = 0; row < h; row++) {
      for (size_t col = 0; col < w; col++) {
          //将图像转换为浮点型填入模型
          data[row * in_stride + col] =
              static_cast<float>(dgd[row * dgd_stride + col]) / max_val;
          //data[row * in_stride + col] = dgd[row * dgd_stride + col] / max_val;
          //cout << data[row * in_stride + col] << "  ";
          assert(input[r * in_stride + c] >= 0.0f);
          assert(input[r * in_stride + c] <= 1.0f);
      }
    }
  ...
  for (auto &item : output_info) {
    ...
    auto output = infer_request.GetBlob(output_name);
    {
      ...
      //cout << "  out channel: " << c << "  height  " << h << "  width  " << w << endl;
      for (int ch = 0; ch < c; ch++) {
        for (int row = 0; row < h; row++) {
          for (int col = 0; col < w; col++) {
            if (ch == 0) {
              r0[row][col] = output_buffer[ch * w * h + row * w + col] * max_val;
              //cout << r0[row][col] << "  ";
            } else {
              r1[row][col] = output_buffer[ch * w * h + row * w + col] * max_val;
            }
          }
        }
      }
  ...
  for (int r = 0; r < height; ++r) {
    for (int c = 0; c < width; ++c) {
      ...
      //cout << +static_cast<uint8_t>(sub_dgr[r][c]) << "  ";
      // src img
    ...
    for (int j = 0; j < rows; j++) {
      ...
      for (int i = start_clow; i < end_clow; i++) {
        for (int j = start_row; j < end_row; j++) {
          ...
          //cout << +static_cast<uint8_t>(repic[i][j])<< " ";
        }
      }
    }
  }
  ...
  for (int r = 0; r < height; ++r) {
    for (int c = 0; c < width; ++c) {
      rst[r * rst_stride + c] = repic[r][c];
      /*cout << "rst "<< + static_cast<uint8_t>(rst[r * rst_stride + c]) << "  ";
      cout << "dgd " << +static_cast<uint8_t>(dgd[r * rst_stride + c]) << "  ";*/
...
//by dgc
extern "C" void av1_openvino(AV1_COMMON *cm, int num_threads,
  ...
  for (int plane = plane_from; plane <= plane_to; ++plane) {
    ...
    if (cm->seq_params.use_highbitdepth) {
      ...
      switch (plane) {
        case AOM_PLANE_Y:
          av1_openvino_restore_cnn_tflite(
              cm->base_qindex, buf->y_buffer, buf->y_crop_width,
              buf->y_crop_height, buf->y_stride, buf->y_buffer, buf->y_stride,
